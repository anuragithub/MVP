app.controller('EmailProcessController', ['$scope','$rootScope','$http', '$state','$stateParams','Upload','ngDialog', function($scope, $rootScope, $http, $state, $stateParams,Upload, ngDialog)  
{
	var uploadURL 	= "";
	...
	{
		uploadURL = 'EmailTemplateCreationServlet';
	}
	...
	$scope.openSFDialog = function() 
	{
		ngDialog.openConfirm({
			...
			$scope.uploadEmailProcess($scope.file, value);
		});
	}
	//
	 //Upload Email Auto Process// to be moved to pop-up
	$scope.uploadEmailProcess = function(file, value) {
	   $scope.f = file;
